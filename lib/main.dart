...
Future<void> requestAllPermissions() async {
  ...
  // Requesting notification permission is crucial for FCM to work on Android 13+
...
Future<void> main() async {
  ...
  // Subscribe to the single topic for all messages

  await FirebaseMessaging.instance.subscribeToTopic("message");



  // Initialize and start the persistent background service
  ...
  try {
    ...
    FirebaseService.instance.setupForegroundListeners(contextService: contextService);
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // The LifecycleWatcher is no longer needed for FCM logic, but might be

    // useful for other things, so we can leave it for now.
  ...
  void secretChage(value) async {
    ...
    if (value.toString().length >= password_text.length) {
      ...
      if (checkPass(value)) {
        ...
        // The service now auto-starts, so no manual call is needed here.
